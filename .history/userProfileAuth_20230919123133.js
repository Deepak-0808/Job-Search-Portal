...
function userAuthFun() {
  ...
  const signinSignupVisibility = document.getElementById(
    "signIn-signup-visibility"
  );
...
accountDelete.addEventListener("click", () => {
  ...
  if (user) {
    ...
    if (confirmation) {
      // If u want to Delete the user's account only. It will not delete user dfrom database 
      user
        .delete()
        .then(() => {
