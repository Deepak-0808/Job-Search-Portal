...
document.getElementById('resume-upload-form').addEventListener('submit', function (e) {
  ...
  if (user) {
    ...
    const storageRef = firebase.storage().ref(`users/${uid}/profile/experience`);
    ...
    uploadTask.on(
      ...
      () => {
        ...
        storageRef.getDownloadURL().then((downloadURL) => {
          ...
          const db = firebase.database();
          const resumeRef = db.ref(`users/${uid}/profile/resume`);
          resumeRef.set(downloadURL).then(() => {
            console.log('Resume URL stored in the database.');
          });
...
// Function to display the user's resume on the profile page
function displayResume() {
  ...
  if (user) {
    ...
    resumeRef.on('value', (snapshot) => {
      ...
      if (downloadURL) {
        // Display the resume URL as a link or embed it in an iframe
...
// Call displayResume() when the user logs in
