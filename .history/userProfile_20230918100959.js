...
document.getElementById('resume-upload-form').addEventListener('submit', function(e) {
  ...
  if (fileInput.files.length > 0) {
      ...
      // Create a unique key for the resume
      var resumeKey = database.ref('resumes').push().key;
      
      // Create a storage reference for the resume
      var storageRef = firebase.storage().ref('resumes/' + resumeKey + '/' + file.name);
      ...
      storageRef.put(file).then(function(snapshot) {
          ...
          storageRef.getDownloadURL().then(function(downloadURL) {
              ...
              database.ref('resumes/' + resumeKey).set({
                  name: file.name,
                  url: downloadURL
              }).then(function() {
...
This JavaScript code adds an event listener to the form submission and handles the file upload and storage in Firebase. It generates a unique key for each uploaded resume and stores the resume's download URL in the Realtime Database under the "resumes" node. Make sure to replace the Firebase initialization code with your actual Firebase configuration.

With this code, when a user selects a resume file and clicks the "Upload Resume" button, the file will be uploaded to Firebase Storage, and its download URL will be stored in Firebase Realtime Database under the "resumes" node with a unique key.
