...
firebase.auth().onAuthStateChanged(function (user) {
  if (user) {
      ...
      // Event listener for the resume form
      document.getElementById("resume-upload-form").addEventListener("submit", function (event) {
          event.preventDefault();

          // Get the selected resume file
          var resumeFile = document.getElementById("resume-file").files[0];

          if (resumeFile) {
              // Create a reference to the user's resume data
              var resumeRef = database.ref('users/' + uid + '/profile/resume');

              // Upload the resume file to Firebase Storage
              var secondaryApp = firebase.initializeApp(config, 'secondary');
              var storage = secondaryApp.storage();
              var storageRef = storage.ref().child('resumes/' + uid + '/' + resumeFile.name);
              // var storageRef = firebase.storage().ref('resumes/' + uid + '/' + resumeFile.name);
              var uploadTask = storageRef.put(resumeFile);

              // Listen for upload completion
              uploadTask.on('state_changed', null, null, function () {
                  // Get the download URL of the uploaded resume
                  storageRef.getDownloadURL().then(function (downloadURL) {
                      // Store the download URL in the user's resume data
                      resumeRef.set({
