...
var database = firebase.database();
        ...
        // Function to add a new portfolio item to the database
        function addPortfolioItem(name, url, imgUrl) {
            var portfolioRef = database.ref('portfolio');
            var newItemRef = portfolioRef.push();
            newItemRef.set({
                name: name,
                url: url,
                imgUrl: imgUrl
...
document.getElementById("workSampleBtn").addEventListener("click", function () {
  var name = prompt("Enter project name:");
  var url = prompt("Enter project URL:");
  var imgUrl = prompt("Enter image URL:");

  if (name && url && imgUrl) {
      // Add the portfolio item to Firebase
      addPortfolioItem(name, url, imgUrl);

      // Create a new list item in the portfolio list
